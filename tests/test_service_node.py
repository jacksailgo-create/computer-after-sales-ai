...
from services.customer_service_backend.agents.service_agent import service_agent_node


async def run_test():
    print("🚀 开始独立测试 ServiceAgent 节点...\n")

    # 1. 伪造 LangGraph 的输入状态 (State)
    # 模拟用户遇到电脑故障，向系统发起提问
    mock_state = {
        "messages": [
            HumanMessage(content="从郑州市人民医院到小米之家")
        ]
    }
    ...
    try:
        ...
        new_state = await service_agent_node(mock_state)
